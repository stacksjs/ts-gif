...
export function GifReader(buf: Buffer): void {
  let p = 0
  ...
  this.decodeAndBlitFrameBGRA = function (frame_num: number, pixels: Uint8Array) {
  ...
  this.decodeAndBlitFrameRGBA = function (frame_num: number, pixels: Uint8Array) {
    const frame = this.frameInfo(frame_num)
    const num_pixels = frame.width * frame.height
    const index_stream = new Uint8Array(num_pixels) // At most 8-bit indices.
    GifReaderLZWOutputIndexStream(
...
function GifReaderLZWOutputIndexStream(code_stream: Buffer, p: number, output: Uint8Array, output_length: number): Uint8Array<ArrayBufferLike> | void {
